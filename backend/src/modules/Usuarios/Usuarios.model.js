...
class Usuario {
    ...
    create = async (body) => {
        ...
        try {
            ...
            body.senha = await bcrypt.hash(body.senha, securityConfig.saltHash);
            // Efetuamos o cadastro
            let data = await col.insertOne(body);
            // Criamos o log
            this.#createLog('create', '', data.ops[0]);
            // Removemos o campo senha do retorno
            delete (data.ops[0].senha);
    ...
    #createLog = (action, dataAntes, dataDepois) => {
        ...
        const actions = {
            create: `Criou usuário ${dataDepois._seq} - ${dataDepois.nome}`,
            update: `Atualizou usuário ${dataAntes._seq} de (${dataAntes.nome}) para (${dataDepois.nome})`,
            delete: `Deletou usuário ${dataAntes._seq} - ${dataAntes.nome}`
        }
